...
def assemble(files):
	...
	data=f.read()
	f.close()
	lines=data.splitlines()		#finding operators   
	for line in lines:	
		line=line.split(' ')		
		opcodes[line[0]]=int(line[1])   #operator with opcode in a dicionary
	...
	for filen in files:								#each file with last file extension filename.asm
